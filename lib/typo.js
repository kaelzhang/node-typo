...
Typo.prototype.template = function (template, params, callback) {
    ...
    return substitute(template, params, this._helpers, function (err, value) {
        if(callback){
            if(err){
                return callback(err);
            }
...
Typo.prototype.register = function(pattern, parser) {
    Typo._register(this._helpers, pattern, parser);
};
...
Typo._register = function (host, pattern, parser){
    if(Object(pattern) === pattern){
        var patterns = pattern;

        for(pattern in patterns){
            parser = patterns[pattern];
            Typo._register(host, pattern, parser);
        }
...
Typo.prototype._clean = function (string) {
    return this.clean ? string.replace(REGEX_CGR, '') : string; 
};
...
mix(Helpers.prototype, require('./helper/rgb'));
