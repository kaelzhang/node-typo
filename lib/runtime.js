...
function template(template, params, helpers, callback) {
  ...
  var parsed = util.isArray(template) ? template : parser.parse(template);
  var series;
  var substituted = [];

  async.parallel(series = parsed.map(function (section, i) {
  ...
  if (substituted.length === parsed.length && substituted.every(function (s) {
