...
// will treated as the only one parameter
// X one helper, more than one parameters 
// X 'a abc,def'      -> { helper: ['a'], param: ['abc', 'def'] }
// X 'a abc, def'     -> { helper: ['a'], param: ['abc', ' def'] } // whitespaces will not be ignored
...
parser.single = function(template) {
    ...
    var splitted = parser.split(template, ' ');
    ...
    // 'a:b' -> ['a', 'b']
    var rest_splitted = parser.split(rest, ':');

    return {
        helper: helper,
        param: rest_splitted[0],
        data: rest_splitted[1],
        source: template
    };
};
...
parser.split = function(template, splitter) {
